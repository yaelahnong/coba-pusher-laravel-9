...
<x-app-layout>
                      ...
                      <div class="flex px-6 pt-6">
                        <div class="flex-1">
                          <div class="flex space-x-3 rtl:space-x-reverse">
                            <div class="flex-none">
                              <div class="h-10 w-10 rounded-full"><img src="http://localhost/pemilu-2024/public/uploader/kandidat/1-1-small.jpeg" alt="" class="w-full h-full object-cover rounded-full"></div>
                            </div>
                      ...
                      <div class="simplebar-offset" style="right: 0px; bottom: 0px;">
                        <div class="simplebar-content-wrapper" tabindex="0" role="region" aria-label="scrollable content" style="height: auto; overflow: hidden scroll;">
                          <div class="simplebar-content" style="padding: 0px;">
                            <div id="roomList" class="divide-y divide-slate-100 dark:divide-slate-700">
                              @foreach ($roomList as $row)
                              <a href="{{ route('chat.show', $row['inbox_uid']) }}" class="block w-full py-5 focus:ring-0 outline-none cursor-pointer group transition-all duration-150 hover:bg-slate-100 dark:hover:bg-slate-600 dark:hover:bg-opacity-70">
                  ...
                  <div class="card-text h-full" data-v-cda99232="">
                    <div class="h-full" data-v-ebbfe379="">
                      <header class="border-b border-slate-100 dark:border-slate-700">
                        <div class="flex py-6 md:px-6 px-3 items-center">
                          <div class="flex-1">
                            <div class="flex space-x-3 rtl:space-x-reverse">
                              <div class="flex-none">
                                <div class="h-10 w-10 rounded-full relative"><span class="bg-secondary-500 status ring-1 ring-white inline-block h-[10px] w-[10px] rounded-full absolute -right-0 top-0"></span><img src="http://localhost/pemilu-2024/public/uploader/kandidat/1-1-small.jpeg" alt="" class="w-full h-full object-cover rounded-full"></div>
                              </div>
                      ...
                      <footer class="md:px-6 px-4 sm:flex md:space-x-4 sm:space-x-2 rtl:space-x-reverse border-t md:pt-6 pt-4 border-slate-100 dark:border-slate-700">
                        ...
                        <form id="messageForm" method="POST" action="{{ route('chat.store', $activeRoom['inbox_uid']) }}" class="flex-1 relative flex space-x-3 rtl:space-x-reverse">
                          @csrf
                          <input type="hidden" name="userIdPenerima" value="{{ $activeRoom['user_id'] }}">
                          <div class="flex-1"><textarea id="messageInput" name="pesan" type="text" placeholder="Type your message..." class="focus:ring-0 focus:outline-0 block w-full bg-transparent border-0 resize-none"></textarea></div>
                          <div class="flex-none md:pr-0 pr-3"><button id="ctaSendMessage" type="button" class="h-8 w-8 bg-slate-900 text-white flex flex-col justify-center items-center text-lg rounded-full"><span class="transform rotate-[60deg]"><svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" aria-hidden="true" role="img" width="1em" height="1em" preserveAspectRatio="xMidYMid meet" viewBox="0 0 24 24" class="iconify iconify--heroicons-outline">
                          ...
                          <div class="simplebar-offset" style="right: 0px; bottom: 0px;">
                            <div class="simplebar-content-wrapper" tabindex="0" role="region" aria-label="scrollable content" style="height: 100%; overflow: hidden scroll;">
                              <div class="simplebar-content" style="padding: 24px;">
                                ...
                                <div class="h-[100px] w-[100px] rounded-full mx-auto mb-4"><img src="http://localhost/pemilu-2024/public/uploader/kandidat/1-1-small.jpeg" alt="" class="block w-full h-full object-cover rounded-full"></div>
                                ...
                                <ul class="grid grid-cols-3 gap-2">
                                  ...
                                  <li class="h-[46px]"><img src="http://localhost/pemilu-2024/public/uploader/kandidat/1-1-small.jpeg" alt="" class="w-full h-full object-cover rounded-[3px]"></li>
                                  <li class="h-[46px]"><img src="http://localhost/pemilu-2024/public/uploader/kandidat/1-1-small.jpeg" alt="" class="w-full h-full object-cover rounded-[3px]"></li>
                                  <li class="h-[46px]"><img src="http://localhost/pemilu-2024/public/uploader/kandidat/1-1-small.jpeg" alt="" class="w-full h-full object-cover rounded-[3px]"></li>
  ...
  <script>
    ...
    window.inboxId = <?php echo json_encode($activeRoom['inbox_uid']); ?>;
    window.userId = <?php echo json_encode($currUser['id']); ?>;
    ...
    messageInput.addEventListener('keypress', function(e) {
      if (e.keyCode === 13 && !e.shiftKey) {
        ...
        const payload = {
          inbox_uid: <?php echo json_encode($activeRoom['inbox_uid']); ?>,
          user_id: <?php echo json_encode(auth()->user()->id); ?>,
          nama: <?php echo json_encode(auth()->user()->name); ?>,
          pesan: messageInputEl.value,
          pesan_created_at: <?php echo json_encode(date('g:i A', strtotime(now()))); ?>,
        };

        window.pesan.push(payload);
            ...
            <div class="flex space-x-2 items-start justify-end group w-full rtl:space-x-reverse">
              <div class="no flex space-x-4 rtl:space-x-reverse">
                  ...
                  <div class="flex flex-col justify-center whitespace-pre-wrap break-all">
                    <div class="text-contrent p-3 bg-slate-300 dark:bg-slate-900 dark:text-slate-300 text-slate-800 text-sm font-normal rounded-md flex-1 mb-1">${payload.pesan}</div><span class="font-normal text-xs text-slate-400">${payload.pesan_created_at}</span>
                  </div>
              </div>
            </div>
          </div>
        ...
        messageForm.submit();
        // axios.post(config('baseUrl') + '/message', {
        //   pesan: messageInputEl.value
        // });
  ...
  <script>
    window.inbox = <?php echo json_encode($roomList); ?>;
    ...
    function renderRoomList(newNotification = 0) {
      ...
      <a href="${'/chat/'+item.inbox_uid}" class="block w-full py-5 focus:ring-0 outline-none cursor-pointer group transition-all duration-150 hover:bg-slate-100 dark:hover:bg-slate-600 dark:hover:bg-opacity-70">
        <div class="flex space-x-3 px-6 rtl:space-x-reverse">
          ...
          <div class="flex-1 text-start flex">
            <div class="flex-1"><span class="block text-slate-800 dark:text-slate-300 text-sm font-medium mb-[2px]">${item.nama}</span><span class="block text-slate-600 dark:text-slate-300 text-xs font-normal">${newNotification ? (`<b>${item.pesan_terbaru || ''}</b>`) : (item.pesan_terbaru || '')}</span></div>
